...
당연한 소리지만 상수의 값은 변경할 수 없으므로, 강제로 변경시 error가 발생한다.
...
* `==` : 동등연산자로 비교 대상 값의 자료형이 다른 경우 강제로 형을 바꾼 뒤에 비교한다. 따라서 좌항과 우항의 자료형은 상관없이 **내용이 같으면** `true`값을 반환하고, 내용이 틀리면 `false` 값을 반환한다.
